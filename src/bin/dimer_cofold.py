...
path.append("/data/ntc/Repository/ViennaRNA-2.6.4/src/bin/")
from remove import vrna_md_t, vrna_param_s
# constant define
...
class vrna_fold_compound_t:
    ...
    strands: int = 0
    cutpoint: int = 0
    nucleotides: Optional[List[vrna_seq_t]]
    ...
    exp_matrices = vrna_mx_pf_t()  # This should be updated to the correct type
    params: Optional[str] = vrna_param_s()  # This should be updated to the correct type
    exp_params = vrna_exp_param_t()  # This should be updated to the correct type
...
from remove import Duplex, expn
...
def vrna_realloc(pointer, size: int):
    return [pointer] * size




def vrna_seq_encode_simple(sequence: str, md: vrna_md_t):
    if sequence and md:
        l = len(sequence)
        S = [0]* (l+2)
...
def vrna_seq_encode(sequence: str, md: vrna_md_t):
    if sequence and md:
...
def set_sequence(obj: vrna_seq_t, string: str, name: str, md: vrna_md_t, options: int):
    obj.name = name if name else None
    obj.string = string.upper()
    obj.length = len(string)

    if options == 0:
        obj.type = VRNA_SEQ_RNA
    obj.encoding = vrna_seq_encode(obj.string, md)
    obj.encoding5 = [0] * (obj.length + 1)
    obj.encoding3 = [0] * (obj.length + 1)

    if md.circ:
        for i in range(obj.length, 0, -1):
            if obj.encoding[i] == 0:
                continue
            obj.encoding5[1] = obj.encoding[i]
            break

        for i in range(1, obj.length + 1):
            if obj.encoding[i] == 0:
                continue
...
def vrna_sequence_add(vc:vrna_fold_compound_t, string:str, options:int) -> int:
    if vc and vc.type == VRNA_FC_TYPE_SINGLE and string:
        ...
        vc.sequence = vrna_realloc(vc.sequence,new_sequence_length)
        vc.sequence[vc.length:vc.length + add_length] = string

        # Reallocate encoding
        new_encoding_length = vc.length + add_length + 2
        vc.sequence_encoding = vrna_realloc(0, new_encoding_length)
        ...
        vc.sequence_encoding2 = vrna_realloc(0, new_encoding_length)
        
        # Dummy placeholder for actual encoding2 adjustment
        enc = vrna_seq_encode_simple(vc.nucleotides[vc.strands - 1].string, vc.params.model_details)
...
VRNA_CONSTRAINT_CONTEXT_EXT_LOOP = 0x01  # Replace with actual value
VRNA_CONSTRAINT_CONTEXT_HP_LOOP = 0x02  # Replace with actual value
VRNA_CONSTRAINT_CONTEXT_INT_LOOP = 0x04  # Replace with actual value
VRNA_CONSTRAINT_CONTEXT_MB_LOOP = 0x10  # Replace with actual value

# Combine them using bitwise OR
VRNA_CONSTRAINT_CONTEXT_CLOSING_LOOPS = (VRNA_CONSTRAINT_CONTEXT_EXT_LOOP |
                                         VRNA_CONSTRAINT_CONTEXT_HP_LOOP |
                                         VRNA_CONSTRAINT_CONTEXT_INT_LOOP |
                                         VRNA_CONSTRAINT_CONTEXT_MB_LOOP)
# Define the context values
VRNA_CONSTRAINT_CONTEXT_INT_LOOP_ENC = 0x08  # Replace with actual value
VRNA_CONSTRAINT_CONTEXT_MB_LOOP_ENC = 0x20  # Replace with actual value

# Combine them using bitwise OR
VRNA_CONSTRAINT_CONTEXT_ENCLOSED_LOOPS = (VRNA_CONSTRAINT_CONTEXT_INT_LOOP_ENC |
                                          VRNA_CONSTRAINT_CONTEXT_MB_LOOP_ENC)



VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS = VRNA_CONSTRAINT_CONTEXT_CLOSING_LOOPS | VRNA_CONSTRAINT_CONTEXT_ENCLOSED_LOOPS

def default_hc_up(fc: vrna_fold_compound_t, options: int) -> None:
    hc = fc.hc

    if options & VRNA_OPTION_WINDOW:
        # No action needed for the window option
        pass
    else:
        n = fc.length
...
VRNA_CONSTRAINT_CONTEXT_NONE = 0

def default_pair_constraint(fc: vrna_fold_compound_t, i: int, j: int) -> int:
    ...
    if fc.type == VRNA_FC_TYPE_SINGLE:
        if ((j - i) < md.max_bp_span) and ((sn[i] != sn[j]) or ((j - i) > md.min_loop_size)):
            ...
            if type in {3, 4}:
                if md.noGU:
                    ...
                    constraint = (VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS &
                                  ~(VRNA_CONSTRAINT_CONTEXT_HP_LOOP | VRNA_CONSTRAINT_CONTEXT_MB_LOOP))
                else:
                    constraint = VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS
            else:
                constraint = VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS
            ...
            if md.noLP:
                # Check if this nucleotide can actually stack with anything or only forms isolated pairs
                if (i > 1) and (j < fc.length) and (((j - i + 2) < md.max_bp_span) or (sn[i - 1] != sn[j + 1])) and md.pair[S[i - 1]][S[j + 1]]:
                    can_stack = VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS

                if (i + 2 < j) and (((j - i - 2) > md.min_loop_size) or (sn[i + 1] != sn[j - 1])) and md.pair[S[i + 1]][S[j - 1]]:
                    can_stack = VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS
        ...
        return
        if (sn[i] != sn[j]) or (((j - i + 1) <= md.max_bp_span) and ((j - i - 1) >= md.min_loop_size)):
            min_score = md.cv_fact * MINPSCORE
            act_score = (fc.hc.type == VRNA_HC_WINDOW) and fc.pscore_local[i][j - i] or fc.pscore[fc.jindx[j] + i]
            if act_score >= min_score:
                constraint = VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS

            if md.noLP:
                # Check if this nucleotide can actually stack with anything or only forms isolated pairs
                if (i > 1) and (j < fc.length) and (((j - i + 2) < md.max_bp_span) or (sn[i - 1] != sn[j + 1])):
                    outer_pscore = (fc.hc.type == VRNA_HC_WINDOW) and fc.pscore_local[i - 1][j - i + 2] or fc.pscore[fc.jindx[j + 1] + i - 1]
                    if outer_pscore >= min_score:
                        can_stack = VRNA_CONSTRAINT_CONTEXT_ALL_LOOPS

                if (i + 2 < j) and (((j - i - 2) > md.min_loop_size) or (sn[i + 1] != sn[j - 1])):
                    inner_pscore = (fc.hc.type == VRNA_HC_WINDOW) and fc.pscore_local[i + 1][j - i - 2] or fc.pscore[fc.jindx[j - 1] + i + 1]
...
def default_hc_bp(fc: vrna_fold_compound_t, options: int) -> None:
    hc = fc.hc

    if not (options & VRNA_OPTION_WINDOW):
        n = fc.length
...
def hc_reset_to_default(vc: vrna_fold_compound_t) -> None:
    hc = vc.hc
    ...
    # Reset the generalized hard constraint feature if necessary
    if hc.f or hc.data:
        if hc.free_data:
...
def hc_update_up(vc: vrna_fold_compound_t) -> None:
    n = vc.length
    hc = vc.hc
    ...
    if hc.type == VRNA_HC_WINDOW:
        ...
        # Unpaired stretch in exterior loop
        hc.up_ext[n + 1] = 0
        for i in range(n, 0, -1):
            hc.up_ext[i] = 1 + hc.up_ext[i + 1] if (hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_EXT_LOOP) else 0

        # Unpaired stretch in hairpin loop
        hc.up_hp[n + 1] = 0
        for i in range(n, 0, -1):
            hc.up_hp[i] = 1 + hc.up_hp[i + 1] if (hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_HP_LOOP) else 0

        # Unpaired stretch in interior loop
        hc.up_int[n + 1] = 0
        for i in range(n, 0, -1):
            hc.up_int[i] = 1 + hc.up_int[i + 1] if (hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_INT_LOOP) else 0

        # Unpaired stretch in multibranch loop
        hc.up_ml[n + 1] = 0
        for i in range(n, 0, -1):
            hc.up_ml[i] = 1 + hc.up_ml[i + 1] if (hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_MB_LOOP) else 0

        # Loop around once more for circular folding
        if vc.strands < 2:
            if hc.mx[n + 1] & VRNA_CONSTRAINT_CONTEXT_EXT_LOOP:
                hc.up_ext[n + 1] = hc.up_ext[1]
                for i in range(n, 0, -1):
                    if hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_EXT_LOOP:
                        hc.up_ext[i] = min(n, 1 + hc.up_ext[i + 1])
                    else:
                        break
            ...
            if hc.mx[n + 1] & VRNA_CONSTRAINT_CONTEXT_HP_LOOP:
                ...
                for i in range(n, 0, -1):
                    if hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_HP_LOOP:
                        hc.up_hp[i] = min(n, 1 + hc.up_hp[i + 1])
                    else:
                        break
            ...
            if hc.mx[n + 1] & VRNA_CONSTRAINT_CONTEXT_INT_LOOP:
                ...
                for i in range(n, 0, -1):
                    if hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_INT_LOOP:
                        hc.up_int[i] = min(n, 1 + hc.up_int[i + 1])
                    else:
                        break
            ...
            if hc.mx[n + 1] & VRNA_CONSTRAINT_CONTEXT_MB_LOOP:
                ...
                for i in range(n, 0, -1):
                    if hc.mx[n * i + i] & VRNA_CONSTRAINT_CONTEXT_MB_LOOP:
                        hc.up_ml[i] = min(n, 1 + hc.up_ml[i + 1])
                    else:
                        break
...
def vrna_hc_init(vc:vrna_fold_compound_t):
    n = vc.length


    # Allocate memory for new hard constraints data structure
    hc = vrna_hc_t()
    hc.type = VRNA_HC_DEFAULT
    hc.n = n
    hc.mx = bytearray((n + 1) * (n + 1) + 1)
    hc.up_ext = [0] * (n + 2)
    hc.up_hp = [0] * (n + 2)
    hc.up_int = [0] * (n + 2)
    hc.up_ml = [0] * (n + 2)
