...
:program:`RNAcofold` - manual page for RNAcofold 2.6.4
...
partition function (pf) and base pairing probability matrix (using the :option:`-p`
switch)
...
.. option:: --output-format=format-character
...
.. option:: -a, --all_pf[=INT]
    ...
    *(default="1")*
    ...
    also contain the free energy of the ensemble as a comment. Using :option:`-a`
    automatically switches on the :option:`-p` option. Base pair probability computations
...
.. option:: -c, --concentrations
    ...
    In addition to everything listed under the :option:`-a` option, read in initial monomer concentrations and compute the expected equilibrium concentrations of the 5 possible species (AB, AA, BB, A, B).
...
.. option:: -f, --concfile=filename
    ...
    :option:`-c` (and thus :option:`-a` and :option:`-p`) options (see above).
...
.. option:: --centroid
    ...
    the :option:`-p` switch. For historical reasons, the centroid structure output is
